...
public:
    ...
    /////////// MY WORK
    // Declaration of operator overide functions for operators ">" and "<"
    bool operator<(const date&);
    bool operator>(const date&);

    // Declaration of operator memeber function "++"
    date operator++(int);

    // Friend function declaration of operator override for outstream "<<" and isntream">>"
...
///////////// MY WORK
// Instream operator overload (member function)
...
// Outstream operator overload (member function)
...
// Overload operator function "++"(member function) to increment date
void operator++(date& x)
{
    x.incrementDate(1);
}

// Overload operator function ">" (member function)
bool date::operator>(const date& rhs)
{
    // Check if year, month, and day are > rhs.year, rhs.month, and rhs.day
    if (year > rhs.year)
        return true;
...
bool date::operator<(const date& rhs)
{
    // Check if hour and minute are < rhs.hour and rhs.minute
    if (year < rhs.year)
        return true;
    else if (year == rhs.year && month < rhs.month)
        return true;
